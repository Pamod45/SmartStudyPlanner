...
// Stores the user's available study times in Firebase and keeps Core Data updated for fast calendar loading.
final class AvailabilitySlotService {
    ...
    // Saves one availability slot remotely, then mirrors it into the local cache.
    ...
    // Pulls all slots for the user and refreshes the local cache used by the study plan screen.
    ...
    // Deletes the slot from Firebase and Core Data. Related sessions are handled by StudyPlanViewModel.
    func delete(id: String) async throws {
        try await db.collection(collection).document(id).delete()
        CoreDataService.shared.deleteAvailabilitySlot(id: id)
    }

    // Converts optional date fields carefully because single-date and date-range slots store different date values.
